...
<body>
	<form action="modifyReg" method="post" enctype="multipart/form-data" name="fmt">
		<table border="1">
			<tr>
				<td>아이디</td>
                        ...
                        <option value=ktwiz
							<c:if test="${data.dd.preferTeam eq 'ktwiz' }">
                              <c:out value="selected" /></c:if>>KT위즈</option>
                              						<option value=nexenheroes
							<c:if test="${data.dd.preferTeam eq 'nexenheroes' }">
                              <c:out value="selected" /></c:if>>넥센히어로즈</option>
                              						<option value=skwyverns
							<c:if test="${data.dd.preferTeam eq 'skwyverns' }">
                              <c:out value="selected" /></c:if>>SK와이번스</option>
                              						<option value=lottegiants
							<c:if test="${data.dd.preferTeam eq 'lottegiants' }">
                              <c:out value="selected" /></c:if>>롯데자이언츠</option>
