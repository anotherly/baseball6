...
<body>
	<table border=1>
	<form action="playerList2" method="post">
	<c:forEach begin="0" end="4" var="i">
			...
			<td colspan="4">
				...
				<select name="list[${i }].kind">
					<c:forEach items="${kindArr }" var="kk">
						<c:choose>
							<c:when test="${kk[0]==isch.list.get(i).kind }">
								<option value="${kk[0] }" selected="selected">ha${kk[1] }</option>
							</c:when>
							<c:otherwise>
		...
		<tr>
			...
			선수검색:<input type="text" name="tttt">
	...
	<c:forEach begin="0" end="4" var="i">
		<tr>
			<td colspan="4">
				...
				<input type="text" name="list[${i }].what" value="${isch.list.get(i).what }" />호호호
