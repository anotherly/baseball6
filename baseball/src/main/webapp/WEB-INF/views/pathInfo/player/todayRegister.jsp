...
<html>
<head>
<p class="kb" align="center" style="background-color: #ffa">선수 기록 입력<br></p>
<meta http-equiv="Content-Type" content="text/html; charset=EUC-KR">
<title>선수 기록 입력</title>
<%
	...
	gameList = teamBean.getGame_num(request);
	yearList = teamBean.getyear(request);
	monthList = teamBean.getmonth(request);
	dayList = teamBean.getday(request);
%>
...
<script type="text/javascript">
	function reLoading(key) {
		...
		var def = "?statnum=" + document.todayRegister.statnum.value;
		var abc = "&playerId=" + res.replace(/[^0-9]/g, "");

		document.todayRegister.action = "todayRegister" + def + abc;
		document.todayRegister.submit();
	}
</script>
</head>
<body>
	<form action="" name="someform" style="background-color: #ffa" width="100%"></form>
	<form action="detailTodayStat" method="post" id="todayRegister"
		name="todayRegister"  style="background-color: #ffa">
		
		<table border="2" style="background-color: #ffa" width="100%">
			<tr>
				<td><strong>기록번호</strong></td>
				<td><input type="hidden" name="statnum"
					value="${data.dd.statnum }">${data.dd.statnum }</td>
			</tr>
			<tr>
				<td><strong>경기번호</strong></td>
				<td>
					<%
						for (int i = 0; i < gameList.size(); i++) {
					%> <%
 	if (gameList.get(i).equals(teamBean.getGame_numState(request))) {
 			out.print("selected");
 		}
 %> <input type="hidden" name="game_num" value="<%=gameList.get(i)%>">
					<%=gameList.get(i)%></td>
			...
			<tr>
				<td><strong>경기정보</strong></td>
				<td><%=teamBean.getteamState(request)%>
			...
			<tr>
				<td colspan="2" align="center"><select id="year" name="year">
						<option value="">==년도 선택==</option>
						<%
							for (int i = 0; i < yearList.size(); i++) {
						%>
						<option value="<%=yearList.get(i)%>"
							<%if (yearList.get(i).equals(teamBean.getyearState(request))) {
					out.print("selected");
				}%>>
						...
						%>
						<option value="<%=monthList.get(i)%>"
							<%if (monthList.get(i).equals(teamBean.getmonthState(request))) {
					out.print("selected");
				}%>>
							<%=monthList.get(i)%></option>
						<%
							}
						%>
						...
						%>
						<option value="<%=dayList.get(i)%>"
							<%if (dayList.get(i).equals(teamBean.getdayState(request))) {
					out.print("selected");
				}%>>
							<%=dayList.get(i)%></option>
						<%
							}
						%>
				</select></td>
			...
			<tr>
				...
				<select id="teamId"
					name="teamId" onchange="javascript:reLoading('teamId')">
						<option value="">
					...
					out.print("selected");
						...
						%>
				</select> <select name="position" onchange="javascript:reLoading('position')">
					...
					out.print("selected");
						...
						%>
				</select> <select name="playerNameNo" id="playerNameNo"
					onchange="javascript:reLoading('playerNameNo')">
						<!-- id="playerNameNo"  -->
				...
				<%-- <input type="hidden" name="teamId" value="<%=teamBean.getteamState(request)%>"> --%>
				</td>
			</tr>

			<tr>
				<td><strong>출장여부</strong></td>
			...
			<tr>
				<td colspan="2" align="center"><strong><strong>수비지표</strong></td>
			</tr>
			<tr>
				<td><strong>오늘의 포지션</strong></td>
			...
			<!-- ---------------------여기까지 수비지표------------------- -->
			<tr>
				<td colspan="2" align="center"><strong>타자지표</strong></td>
			</tr>
			<tr>
				<td><strong>타석</strong></td>
				<td><input name="ab" value="0" style="border: 0; height: 30px;"
					readonly="true" size="50" /></td>
			</tr>
			<tr>
				<td><strong>득점</strong></td>
			...
			<tr>
				<td><strong>2루타</strong></td>
				<td><input name="twoBase" value="0"
					style="border: 0; height: 30px;" readonly="true" size="50" /></td>
			</tr>
			<tr>
				<td><strong>3루타</strong></td>
			...
			<tr>
				<td><strong>타점</strong></td>
				<td><input name="rbi" value="0"
					style="border: 0; height: 30px;" readonly="true" size="50" /></td>
			</tr>
			<tr>
				<td><strong>볼넷</strong></td>
			...
			<!-- ---------------------------여기까지 타자지표---------- -->
			<tr>
				<td colspan="2" align="center"><strong>투수지표</strong></td>
			</tr>
			<tr>
				<td><strong>승리</strong></td>
			...
			<tr>
				<td><strong>피홈런</strong></td>
				<td><input name="pitcherHr" value="0"
					style="border: 0; height: 30px;" readonly="true" size="50" /></td>
			</tr>
			<tr>
				<td><strong>볼넷허용</strong></td>
				<td><input name="pitcherBb" value="0"
					style="border: 0; height: 30px;" readonly="true" size="50" /></td>
			</tr>
			<tr>
				<td><strong>탈삼진</strong></td>
