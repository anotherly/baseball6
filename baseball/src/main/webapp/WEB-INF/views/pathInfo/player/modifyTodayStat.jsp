...
<%
	...
	ArrayList getYear = new ArrayList();
	ArrayList getMonth = new ArrayList();
	ArrayList getDay = new ArrayList();
	ArrayList getModi = new ArrayList();
	getYear = teamBean.getyear(request);
	getMonth = teamBean.getmonth(request);
	getDay = teamBean.getday(request);
	getModi = teamBean.getModi(request);
%>
...
<script type="text/javascript">
	function reLoading(key) {
		if (key == "year") {
			var set1 = document.getElementsByName("month");
			var set2 = document.getElementsByName("day");
			var set2 = document.getElementsByName("modi");
			set1[0].selectedIndex = 0;
			set2[0].selectedIndex = 0;
			set3[0].selectedIndex = 0;
		}
...
<body>
	...
	<form action="detailTodayStat" method="post" id="todayRegister" name="todayRegister">
		...
		<tr>
				<td>기록번호</td>
				<td><input type="hidden" name="statnum" value="${data.dd.statnum }">${data.dd.statnum }</td>
			</tr>
			<tr>
